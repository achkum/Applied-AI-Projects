...
from token_saver.optimizer import optimize_payload
from token_saver.providers import resolve, resolve_by_path
from token_saver.providers.base import ProviderAdapter
from token_saver.proxy.stats_page import render_stats_html
...
def _log_results(app: FastAPI, results: list[OptimizationResult]) -> None:
    if not results:
        return
    log_dir = Path(os.getenv("TS_LOG_DIR", str(Path.home() / ".token-saver")))
    try:
...
def app_factory(config: OptimizerConfig | None = None) -> FastAPI:
    ...
    app = FastAPI(title="token-saver proxy")

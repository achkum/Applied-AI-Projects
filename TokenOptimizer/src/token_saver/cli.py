"""Command-line entry point: `token-saver start | download-model | stats | mcp`."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"token-saver {__version__}")
...
def _cmd_start(args: argparse.Namespace) -> int:
    ...
    print(f"token-saver proxy listening on {base}")
...
def _cmd_download(args: argparse.Namespace) -> int:
    ...
    print("Downloading the LLMLingua-2 ONNX classifier model…")
    ClassifierCompressor().ensure_model()
    print("Done.")
    return 0
...
def _cmd_stats(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(
            f"Could not reach token-saver at {url}. Is it running? Start it with `token-saver start`.",
            file=sys.stderr,
        )
        return 1
...
def _cmd_mcp(args: argparse.Namespace) -> int:
    from token_saver.mcp_server import main as mcp_main
...
def _cmd_web(args: argparse.Namespace) -> int:
    ...
    from token_saver.webapp import app_factory

    print(f"token-saver engine demo on http://{args.host}:{args.port}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command is None:
        print(f"token-saver {__version__}")
        return 0
    return {
        "start": _cmd_start,
